...
2. On `UI_CAL`, press **`READ REF`** for that channel — this runs the full automated measurement via `meterAtColor()` (the same path as the daily `MEAS BLK`/`MEAS LIT`): the head is switched to that channel's color (`CMD_METER_BLUE_ON`/`CMD_METER_GREEN_ON`), the LEDs warm up for `kMeterWarmupMs` (3 s), an averaged reading is taken (§6), and the light is switched back off — capturing the intensity into `calRefIntensityBlue`/`Green`. No manual color selection is needed anymore; calibration and daily metering see identical, warmed-up light conditions.
3. Manually expose a real test print through the wedge, for the fixed **`calTestHalfSeconds`** duration shown on screen (automating this test exposure via `CMD_EXPOSE_*` remains on the roadmap, see §10).
...
1. **`MEAS BLK`**: sends `CMD_METER_BLUE_ON`, waits `kMeterWarmupMs` (3 s) for the LEDs to warm up, takes one averaged `fetchFreshSensorMetric()` reading (20 samples, §6), sends `CMD_STOP` (`meterAtColor()`). Then:
   ```
...
1. Records current backlight duty, then forces backlight to `0` and waits `1000 ms` before triggering the sensor samples — the panel's own light must not contaminate an optical reading of the print/negative area.
2. Takes up to `kMeterSamples` (20) individual samples, each via `fetchSensorSampleRaw()`: send `PKT_TYPE_CMD_EXT_SAMPLE_NOW` to the RP2040 bridge, poll up to `kMeterSampleTimeoutMs` (1400 ms) for a fresh, valid sample. **Sensor priority is fixed per sample, not "whichever reported last":** `TSL2591` lux is always preferred when present (its on-bridge auto-gain makes it reliably usable without spectral calibration, and the ratio-based exposure math in §5 doesn't need one); the `AS7343` blue/green mix is only used as a fallback when the bridge reports no TSL2591 reading. This matters because the bridge sends both readings every cycle whenever both sensors are attached — without an explicit priority, whichever sensor's packet happens to arrive last would silently "win" (in practice always AS7343, since it's sent second on the bridge).
3. **Averaging and failure rules:** the whole run is capped by `kMeterRunBudgetMs` (8 s). If the very first sample times out (dead link / missing sensor), the run aborts immediately so the screen doesn't stay dark for the whole budget. At least `kMeterSamplesMin` (12) valid samples are required; the result is a trimmed mean (single min and max dropped) of what was collected. With the TSL2591's fixed 100 ms integration time a full 20-sample run takes ~2.5 s. Multi-sample averaging matters because these readings end up persisted in the calibration doses (§5.2) — a single noisy sample would otherwise sit in flash until the next recalibration. Calibration (`READ REF`) and daily metering (`MEAS BLK`/`MEAS LIT`) intentionally share this exact same averaging so the noise statistics cancel in the `dose / intensity` ratio.
4. Restores the backlight duty **on every exit path**, including all failure paths. The only path that does not touch backlight at all is the very first guard (`!rpLinkReady`), which is safe because backlight was never turned off in that case.
5. This call is **blocking** (~3.5 s typical incl. the 1 s display settle; worst case bounded by the 8 s budget) and is only ever invoked from paths gated on `runState == RUN_IDLE`, so it never interrupts or delays an active print exposure.

`meterAtColor(meterOnCmd, metricOut)` (§5.3) is a thin wrapper around this same function for all color measurements: it sends the relevant `CMD_METER_BLUE_ON`/`CMD_METER_GREEN_ON`, waits `kMeterWarmupMs` (3 s) for the LEDs to warm up, calls `fetchFreshSensorMetric()` unchanged, then sends `CMD_STOP`. Since the automated-calibration change, the calibration screen's `READ REF` action goes through this same wrapper (blue on the left column, green on the right) instead of reading the sensor with whatever light happened to be on — the head's single-color guarantee (§3) now covers calibration too.
...
3. **Manual test-print exposure during calibration.** `READ REF` now drives the head's single-color light itself (§5.2), so the reference measurement no longer depends on manual color selection. The calibration test print, however, is still exposed manually by the user at their real enlarger — automating it via `CMD_EXPOSE_*` (§10) hasn't been done yet, so ensure the correct single color manually for that step.
...
5. **Blocking measurement call** (~7–8 s typical: 3 s LED warm-up + 1 s display settle + ~2.5 s for 20 samples; worst case bounded by the 8 s sampling budget) freezes the sender's UI/loop during `MEAS BLK`/`MEAS LIT`/`READ REF`. Acceptable since it's always a deliberate, `RUN_IDLE`-gated action, but would need to become asynchronous if future features require touch responsiveness during measurement.
